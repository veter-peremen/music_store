...
/** Жанры музыканта — вычисляются из его пластинок, не хранятся (ТЗ §3). */
router.get(
  '/:id/genres',
  asyncHandler(async (req, res) => {
...
/** Архивация музыканта каскадно архивирует его пластинки (ТЗ §4.5). */
router.delete(
  '/:id',
  asyncHandler(async (req, res) => {
    const id = parse(idParam, req.params.id);
...
/** Возврат из архива: только сам музыкант, пластинки восстанавливаются отдельно (ТЗ §4.5). */
router.post(
  '/:id/restore',
  asyncHandler(async (req, res) => {
    const id = parse(idParam, req.params.id);
