...
Без входа данные недоступны ни на чтение, ни на запись — иначе `401 UNAUTHORIZED`. Открыты только `/auth/*`, `/health`, `/health/ready`, индекс `/api` и файлы интерфейса. Вошедшим читать можно всем; менять — ролям `staff` и `superadmin`, наблюдателю придёт `403 FORBIDDEN`.
...
Доступно только роли `superadmin` — остальным, включая чтение списка, приходит `403 FORBIDDEN`.

| Метод  | Путь         | Описание                                    |
| ------ | ------------ | ------------------------------------------- |
| GET    | `/users`     | список учётных записей                      |
| POST   | `/users`     | создать: `{ "login", "password", "role" }`  |
| PATCH  | `/users/:id` | сменить роль: `{ "role": "staff" }`         |
| DELETE | `/users/:id` | удалить учётную запись вместе с её сессиями |

Роли: `superadmin` (управляет учётными записями и данными), `staff` (меняет данные магазина), `viewer` (только просмотр). Саморегистрация через `/auth/register` всегда даёт `viewer`; другую роль назначает суперадминистратор.

Ограничители, чтобы не остаться без доступа:

| Код               | Когда                                      |
| ----------------- | ------------------------------------------ |
| `SELF_DELETE`     | попытка удалить собственную учётную запись |
| `SELF_DEMOTE`     | попытка снять права с самого себя          |
| `LAST_SUPERADMIN` | попытка удалить или понизить последнего    |

Первого суперадминистратора через API назначить нельзя — менять роли может только суперадминистратор. Для первого запуска есть скрипт:

```bash
npm run grant-role -- superadmin superadmin
```
...
| Метод | Путь   | Описание                                              |
| ----- | ------ | ----------------------------------------------------- |
| GET   | `/api` | название сервиса, версия и список доступных маршрутов |
...
Сам корень `/` занят веб-интерфейсом — это обычные страницы, а не API. Адреса ресурсов (`/records`, `/sales` и прочие) от этого не изменились.
